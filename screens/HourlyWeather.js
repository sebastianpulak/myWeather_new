    ...
    if (this.state.dataSource.cod == 200) {
      ...
      for (let i = 0; i < this.state.dataSource.list.length - 1; i++) {
        if (!this.state.dataSource.list[i].rain) {
          row.push(
            <View style={styles.viewColumn} key={i}>
              <TouchableOpacity style={styles.tile} key={i} onPress={() => this.goToDetails(this.state.dataSource.list[i], this.state.cityName)}>
                ...
                <View>
                <Text style={styles.tileTextName}> {Math.round(this.state.dataSource.list[i].main.temp)}℃
                    <Image style={{ height: 38, width: 38 }} source={{ uri: this.state.iconUrl + this.state.dataSource.list[i].weather[0].icon + '.png' }}></Image>       Rain: 0mm</Text>
                </View> 
              </TouchableOpacity>
            </View>
          );
          ...
          row.push(
            <View style={styles.viewColumn} key={i}>
              <TouchableOpacity style={styles.tile} key={i} onPress={() => this.goToDetails(this.state.dataSource.list[i], this.state.cityName)}>
                ...
                <View>
                  <Text style={styles.tileTextName}> {Math.round(this.state.dataSource.list[i].main.temp)}℃
                    <Image style={{ height: 38, width: 38 }} source={{ uri: this.state.iconUrl + this.state.dataSource.list[i].weather[0].icon + '.png' }}></Image>       Rain: {this.state.dataSource.list[i].rain["3h"]}mm</Text>
                </View>
              </TouchableOpacity>
            </View>
          );
